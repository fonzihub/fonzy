...
public class MailgunSender {
    ...
    public void sendBillingEmail(User recipient, Abbonamento abb) {
        HttpResponse<JsonNode> response = Unirest.post("https://api.mailgun.net/v3/" + this.domain + "/messages")
                .basicAuth("api",this.apiKey)
                .queryString("from","brad@sandbox244a1f5e40eb4904980a949ae26268c6.mailgun.org")
                .queryString("to",recipient.getEmail())
                .queryString("subject","Registrazione completata")
                .queryString("text","Ciao, " + recipient.getUsername() + ", il tuo abbonamneto a " +
                        abb.getCatalogo().getName() + " scade il " + abb.getDataRinnovo() + ". Ricordati di rinnovarlo! ")
                .asJson();
